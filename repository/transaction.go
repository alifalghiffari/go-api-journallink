...
func (u *TransactionRepository) Pay(cartItems []CartItem, amount int) (int, error) {
	...
	for _, cartItem := range cartItems {
		...
		// TODO: answer here
		queryUpdateQuantity := `UPDATE products SET quantity = quantity - ? WHERE id = ?;`

		_, err = tx.Exec(queryUpdateQuantity, cartItem.Quantity, cartItem.ProductID)
		if err != nil {
			tx.Rollback()
			return 0, err
		}
		...
		// HINT: use `tx.Exec("... query ...")` instead of `u.db.Exec("... query ...")`
		// TODO: answer here
		queryInserSales := `INSERT INTO sales (date, product_id, quantity) VALUES (?, ?, ?);`

		_, err = tx.Exec(queryInserSales, time.Now(), cartItem.ProductID, cartItem.Quantity)
		if err != nil {
			tx.Rollback()
			return 0, err
		}
